...
The README and `specs/README.md` use Markdown tables as a structured index — a "lookup table"
pattern that maps each system to its spec file and implementing crates.

The `specs/README.md` is the canonical index. It's organized into categories, each with a table:
...
|-------|-----------------|
| **README lookup tables** | Markdown tables in `specs/README.md` mapping spec → code → purpose |
