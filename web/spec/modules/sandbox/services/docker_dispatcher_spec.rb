...
RSpec.describe Sandbox::DockerDispatcher, type: :service do
  ...
  describe "#dispatch" do
    it "returns exit_code 0 and stdout on success" do
      ...
      result = dispatcher.dispatch(image: "ruby:3.3-slim", command: ["echo", "hello world"])
    ...
    it "returns non-zero exit_code without raising on failure" do
      ...
      result = dispatcher.dispatch(image: "ruby:3.3-slim", command: ["false"])
    ...
    it "does not log env vars containing Secret values" do
      ...
      dispatcher.dispatch(image: "ruby:3.3-slim", command: ["env"], env: { "API_KEY" => secret, "PLAIN" => "visible" })
    ...
    it "kills container and returns non-zero exit on timeout" do
      ...
      result = short_dispatcher.dispatch(image: "ruby:3.3-slim", command: ["sleep", "999"])
    ...
    it "creates a ContainerRun record and updates it with final status" do
      ...
      expect {
        dispatcher.dispatch(image: "ruby:3.3-slim", command: ["echo", "ok"])
    ...
    it "passes command as argument array — no shell interpolation" do
      ...
      dispatcher.dispatch(image: "ruby:3.3-slim", command: ["echo", "hello; rm -rf /"])
