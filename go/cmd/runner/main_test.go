...
func TestSendAnalyticsEvent(t *testing.T) {
	var received analyticsEvent
	var callCount int

	mockAnalytics := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		callCount++
		if req.URL.Path != "/capture" {
			t.Errorf("expected path /capture, got %s", req.URL.Path)
		}
	...
	r := newRunnerWithRegistry("http://localhost", mockAnalytics.URL, "testuser", "testpass", prometheus.NewRegistry())
	r.sendAnalyticsEvent("aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee", 0, 150, 320, 42.5)

	if callCount != 1 {
		t.Fatalf("expected 1 analytics call, got %d", callCount)
	}
	if received.EventName != "agent_run_completed" {
		t.Errorf("expected event_name agent_run_completed, got %s", received.EventName)
	}
	if received.DistinctID != "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee" {
		t.Errorf("expected distinct_id to be agent_run_id, got %s", received.DistinctID)
	}
	if received.OrgID == "" {
...
func TestSendAnalyticsEventSkipsWhenNoURL(t *testing.T) {
	r := newRunnerWithRegistry("http://localhost", "", "testuser", "testpass", prometheus.NewRegistry())
	// Should return immediately without error when analyticsURL is empty.
	r.sendAnalyticsEvent("aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee", 0, 150, 320, 42.5)
}

func TestSendAnalyticsEventHandlesServerError(t *testing.T) {
	mockAnalytics := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
	...
	r := newRunnerWithRegistry("http://localhost", mockAnalytics.URL, "testuser", "testpass", prometheus.NewRegistry())
	// Should not panic on server error — fire-and-forget.
	r.sendAnalyticsEvent("aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee", 1, 0, 0, 1.0)
}
