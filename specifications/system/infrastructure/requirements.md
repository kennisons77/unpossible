...
- **Projects built with unpossible:** each project gets its own compose stack generated
  by the loop from `specifications/project-requirements.md`. Infrastructure is not shared between projects.
...
The loop reads `specifications/project-requirements.md` for a new project, finds the language and base image, and
...
- As the build loop, I want to generate `infra/Dockerfile` and `infra/docker-compose.yml`
  for new projects from `specifications/project-requirements.md` so projects are runnable without manual setup.
...
- **Project bootstrap** — loop generates `infra/Dockerfile` and `infra/docker-compose.yml`
  for each project from `specifications/project-requirements.md` (language, base image, test command, port).
...
|---|---|
| [`concept.md`](concept.md) | Phase model, compose file layout, pod layout, NixOS module structure, file ownership table, acceptance criteria |
